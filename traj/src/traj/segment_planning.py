...
def equal_vel_case_planning(pos_diff, v, abs_max_vel, abs_max_acc, abs_max_jrk):
    ...
    min_pos_to_max_vel, reached_acc_to_max_vel = calculate_min_pos_reached_acc_to_reach_max_vel(v, abs_max_vel,
                                                                                                abs_max_acc,
                                                                                                abs_max_jrk)
    ...
    # if  no: check if a const_acc phase is required or not (to satisfy pos_diff)
        ...
        min_pos_to_max_vel, t_max_acc = calculate_min_pos_const_acc_time_to_reach_max_acc_and_max_vel(v, abs_max_vel,
                                                                                                      abs_max_acc,
                                                                                                      abs_max_jrk)
        reached_vel = abs_max_vel
        ...
        if (pos_diff >= min_pos_to_max_vel):
            ...
            if (pos_diff >= min_pos_to_max_acc):
                ...
                acc = calculate_reached_acc_in_case_no_const_acc_phase(pos_diff, v, abs_max_vel, abs_max_acc,
                                                                       abs_max_jrk)
                t_max_jrk = acc / abs_max_jrk
                t_max_acc = 0.0
                t_max_vel = 0.0
...
def traj_segment_planning(p_start, p_end, abs_v_start, abs_v_end, abs_max_vel, abs_max_acc, abs_max_jrk):
    ...
    abs_min_pos_to_vf, acc_to_vf, t_jrk_to_vf, t_acc_to_vf = calculate_min_pos_reached_acc_jrk_time_acc_time_to_reach_final_vel(
        abs_v_start, abs_v_end, abs_max_vel, abs_max_acc, abs_max_jrk)
    if abs_min_pos_to_vf > abs(p_end - p_start) and abs_min_pos_to_vf - abs(
            p_end - p_start) > 1e-5:  # if abs_min_pos_to_vf> abs(p_end-p_start), then these values are not feasible
        ...
        if abs(abs(p_end - p_start) - abs_min_pos_to_vf) > 1e-7:
            ...
            t_jrk, t_acc, t_vel, reached_vel, reached_acc = equal_vel_case_planning(abs(pos_diff), abs_v, abs_max_vel,
